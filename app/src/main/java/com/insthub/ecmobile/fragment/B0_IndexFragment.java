...
//        title.setText(ecmobileStr);
        ...
        (getActivity()).getActionBar().setTitle("送至：重庆市江北区观音桥");
    ...
    public void onResume() {
		...
		ConfigModel configModel = new ConfigModel(getActivity());
        configModel.getConfig();
        EventBus.getDefault().register(this, "onHomeClick", HomeClickEvent.class);
    ...
    public void homeSetAdapter() {
    	if(dataModel.homeDataCache() != null) {
          if (null == listAdapter)
          {
              listAdapter = new B0_IndexAdapter(getActivity(), dataModel,searchModel);
...
//            parentListAdapter.notifyDataSetChanged();
            ...
            if (s.succeed == 1){
                ...
                if (addressChoiceDialog == null){
                    addressChoiceDialog = new AddressChoiceDialog(getActivity(),"请选择送货地址",addressModel.addressList);
                    addressChoiceDialog.positive.setOnClickListener(new OnClickListener() {
                        @Override
    ...
    public void onHomeClick(HomeClickEvent event){
