...
public class LoginModel extends BaseModel {
    ...
    public void getCode(String phoneNum){
        Log.e("hshs","已经发送短信"+phoneNum);
        BeeCallback<JSONObject> cb = new BeeCallback<JSONObject>(){
            @Override
            public void callback(String url, JSONObject jo, AjaxStatus status) {
                LoginModel.this.callback(url, jo, status);
                Log.e("hshs","已经发送短信"+jo.toString());
            }
        };
        Map<String,String> params = new HashMap<String, String>();
        JSONObject jo = new JSONObject();
        try {
            jo.put("telephone",phoneNum);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        params.put("json",jo.toString());
        cb.url(ApiInterface.USER_SMS_SEND).type(JSONObject.class).params(params);
        Log.e("hshs", "aq.ajax(cb)");
        aq.ajax(cb);
    }
    ...
    public void login(String name, String password) {
        ...
        request.telephone = name;
        request.sms_password= password;
        Map<String, String> params = new HashMap<String, String>();
        try {
        ...
        cb.url(ApiInterface.USER_SMS_SIGNIN).type(JSONObject.class).params(params);
